...
main() {
  ...
  if ! trivy image --scanners vuln --timeout 30m --format json -o "${OUT_DIR}/${SAFE_NAME}-before.json" "${IMAGE}"; then
...
EOF
  ...
  if docker build -t "${PATCHED_LOCAL_TAG}" "${BUILD_DIR}" \
    && trivy image --scanners vuln --timeout 30m --format json -o "${OUT_DIR}/${SAFE_NAME}-after.json" "${PATCHED_LOCAL_TAG}"; then
